...
if (fire_mid && t_mid != -4)
{
    scr_launch_bullet(x + 40, y - 75, y, grid_row, px_mid, t_mid);
}
...
if (fire_up)
{
    ...
    {
        if (t_mid != -4)
            scr_launch_bullet(x + 20, y - 75, y, grid_row, px_mid, t_mid);
    }
    else
    {
        if (t_up != -4)
            scr_launch_bullet(x + 40, y - 75 - _lane_h, y - _lane_h, grid_row - 1, px_up, t_up);
    }
}
...
if (fire_down)
{
    ...
    {
        if (t_mid != -4)
            scr_launch_bullet(x + 60, y - 75, y, grid_row, px_mid, t_mid);
    }
    else
    {
        if (t_down != -4)
            scr_launch_bullet(x + 40, (y - 75) + _lane_h, y + _lane_h, grid_row + 1, px_down, t_down);
    }
}
